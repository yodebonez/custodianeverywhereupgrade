...
namespace CustodianEveryWhereV2._0.Controllers
{
    ...
    public class FitfamPlusController : ControllerBase
    {
        ...
        public FitfamPlusController(IConfiguration configuration, IWebHostEnvironment hostingEnvironment)
        {
        ...
        public async Task<notification_response> GetDeals(string merchant_id)
        {
            try
            {
                ...
                var endpoint = _configuration["Settings:ENDPOINTS"].Split('|');
        ...
        [HttpGet("{phone?}/{merchant_id?}/{hash?}")]
        ...
        public async Task<notification_response> BuyDeal(deal deal)
        {
            ...
            try
            {
                ...
                string jsonFilePath = Path.Combine(_hostingEnvironment.WebRootPath, "Cert", "config.json");          
                string jsonContent =System.IO.File.ReadAllText(jsonFilePath);            
                List<dynamic> configList = JsonSerializer.Deserialize<List<dynamic>>(jsonContent);         
                dynamic geturl = configList.FirstOrDefault(x => x.gymname == deal.gym.ToUpper().Trim());
