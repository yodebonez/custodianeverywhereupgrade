...
namespace CustodianEveryWhereV2._0.Controllers
{
    ...
    public class MealPlanController : ControllerBase
    {
        ...
        public MealPlanController(IConfiguration configuration)
        {
                        ...
                        foreach (var item in group_plan)
                        {
                            ...
                            foreach (var subitem in meallist)
                            {
                                day.Add(subitem.First().mealType, subitem.Select(x => new temp
                                {
                                    ...
                                    time = x.time,
                                    youtubeurl = x.youTubeUrl,
                                    image_path = $"{_configuration["Settings:MEAL_PLAN_IMAGES"]}/{x.image}"
                                }).ToList());
                            }
                            dic.Add(item.First().daysOfWeek.Trim(), day);
                        }
        ...
        /// <returns></returns>
        [HttpGet("{emailOrphone?}/{merchant_id?}/{hash?}")]
                        ...
                        foreach (var item in group_plan)
                        {
                            ...
                            foreach (var subitem in meallist)
                            {
                                day.Add(subitem.First().MealPlan.mealType, subitem.Select(x => new temp
                                {
                                    ...
                                    image_path = $"{_configuration["Settings:MEAL_PLAN_IMAGES"]}/{x.MealPlan.image}"
                                }).ToList());
                            }
        ...
        /// <param name="merchant_id"></param>
        /// <returns></returns>
        [HttpGet("{emailOrphone?}/{merchant_id?}")]
