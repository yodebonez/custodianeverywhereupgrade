...
namespace CustodianEveryWhereV2._0.Controllers
{
    ...
    public class InterstateController : ControllerBase
    {
        ...
        public InterstateController(IWebHostEnvironment hostingEnvironment)
        {
        ...
        public async Task<res> OnboardingInterstate(InterState interState)
        {
            try
            {
               ...
               // var getPrivateKeyPath = HttpContext.Current.Server.MapPath("~/Cert");
                // Map path for the certificate folder
                string getPrivateKeyPath = Path.Combine(_hostingEnvironment.ContentRootPath, "Cert");
                ...
                using (var api = new HttpClient())
                {
                    ...
                    {
                        var response = await request.Content.ReadFromJsonAsync<InterstateResponse>();
        ...
        public async Task<res> GetFormSelections()
        {
            try
            {
                ...
                string country_file = System.IO.File.ReadAllText(jsonFilePath);
                ...
                string jsonFilePathh = Path.Combine(_hostingEnvironment.WebRootPath, "TravelCategoryJSON", "countryPhoneCode.json");

                // Read the contents of the file
                string country_phone_code = System.IO.File.ReadAllText(jsonFilePathh);
