...
namespace CustodianEveryWhereV2._0.Controllers
{
    ...
    public class MoviesController : ControllerBase
    {
        ...
        //private  string base_url = _configuration["Settings:MOVIE_API"];//https://api.themoviedb.org/3
        //private string movie_api_key = _configuration["Settings:MOVIE_API_KEY"]; //e26d2c39816a704c232faacd23e1c2cc
        //private string image_url = _configuration["Settings:MOVIE_API_KEY"]; //https://image.tmdb.org/t/p/w500

      

        [HttpGet("{hash?}/{merchant_id?}/{region?}/{page?}")]
        public async Task<notification_response> UpComingMovies(string hash, string merchant_id, string region = "", int page = 1)
        {

            var base_url = _configuration["Settings:MOVIE_API"];
           var movie_api_key = _configuration["Settings:MOVIE_API_KEY"];
            var image_url = _configuration["Settings:MOVIE_API_KEY"];
            ...
            try
            {
                ...
                {
                    var url = (string.IsNullOrEmpty(region)) ? base_url + $"/movie/upcoming?api_key={movie_api_key}&language=en-US&page={page}" :
                        base_url + $"/movie/upcoming?api_key={movie_api_key}&language=en-US&page={page}&region={region}";
                    var req = await api.GetAsync(url);
                    if (req.IsSuccessStatusCode)
                    {
                        var res = await req.Content.ReadFromJsonAsync<dynamic>();
                        if (res.results != null)
                        {
                            return new notification_response
                            {
        ...
        [HttpGet("{hash?}/{merchant_id?}/{movie_id?}")]
        public async Task<notification_response> GetMovieDetails(string hash, string merchant_id, int movie_id)
        {
            ...
            try
            {
                ...
                using (var api = new HttpClient())
                {
                    ...
                    var url = base_url + $"/movie/{movie_id}?api_key={movie_api_key}&language=en-US&append_to_response=videos";
                    var req = await api.GetAsync(url);
                    if (req.IsSuccessStatusCode)
                    {
                        var res = await req.Content.ReadFromJsonAsync<dynamic>();
        ...
        [HttpGet("{hash?}/{merchant_id?}/{movie_id?}/{page?}")]
        public async Task<notification_response> GetSimilarMovies(string hash, string merchant_id, int movie_id, int page = 1)
        {
            ...
            try
            {
                ...
                using (var api = new HttpClient())
                {
                    ...
                    var url = base_url + $"/movie/{movie_id}/similar?api_key={movie_api_key}&language=en-US&page={page}";
                    var req = await api.GetAsync(url);
                    if (req.IsSuccessStatusCode)
                    {
                        var res = await req.Content.ReadFromJsonAsync<dynamic>();
        ...
        [HttpGet("{hash?}/{merchant_id?}/{query?}")]
        ...
        public async Task<notification_response> GetTVSeries(string merchant_id, string hash, int page = 1)
        {
            ...
            try
            {
                ...
                using (var api = new HttpClient())
                {
                    ...
                    //https://api.themoviedb.org/3/movie/now_playing?api_key=e26d2c39816a704c232faacd23e1c2cc&language=en-US&page=1
                    var url = base_url + $"/tv/popular?api_key={movie_api_key}&language=en-US&page={page}";
                    var req = await api.GetAsync(url);
                    if (req.IsSuccessStatusCode)
                    {
                        var res = await req.Content.ReadFromJsonAsync<dynamic>();
        ...
        [HttpGet("{merchant_id?}/{hash?}/{movie_series_id?}/{page?}")]
        public async Task<notification_response> GetTVSeriesDetails(string merchant_id, string hash, int movie_series_id, int page = 1)
        {
            ...
            try
            {
                ...
                using (var api = new HttpClient())
                {
                    ...
                    var url = base_url + $"/tv/{movie_series_id}?api_key={movie_api_key}&language=en-US&append_to_response=videos";
                    var req = await api.GetAsync(url);
                    if (req.IsSuccessStatusCode)
                    {
                        var res = await req.Content.ReadFromJsonAsync<dynamic>();
