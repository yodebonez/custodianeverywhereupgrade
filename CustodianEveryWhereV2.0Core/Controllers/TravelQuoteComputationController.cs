...
namespace CustodianEveryWhereV2._0.Controllers
{
    ...
    public class TravelQuoteComputationController : ControllerBase
    {
        ...
        public TravelQuoteComputationController(IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
        ...
        public async Task<notification_response> GetQuote(Quote quote)
        {
            ...
            try
            {
                ...
                // need to get value for exchange rate travel quote
                double exchnageRate = Convert.ToDouble(_configuration["AppSettings:TRAVEL_EXCHANGE_RATE"]);
        ...
        public async Task<notification_response> GetCountry()
        {
            try
            {
              //  var country_file = System.IO.File.ReadAllText(HttpContext.Current.Server.MapPath("~/TravelCategoryJSON/Country.json"));
                ...
                string jsonFilePath = Path.Combine(_hostingEnvironment.WebRootPath, "TravelCategoryJSON", "Country.json");

                // Read the contents of the file
                string countryFile = System.IO.File.ReadAllText(jsonFilePath);

                var country_list = Newtonsoft.Json.JsonConvert.DeserializeObject<dynamic>(countryFile);
                return new notification_response
                {
                    status = 200,
                    message = "successful",
        ...
        public async Task<notification_response> BuyTravelInsurance(BuyTravel travel)
        {
            ...
            try
            {
                ...
                //image directory
                string dir = $"{_configuration["Settings:DOC_PATH"]}/Documents/Travel/{group_reference}";
                ...
                {
                    var filepath = $"{_configuration["Settings:DOC_PATH"]}/Documents/Travel/{group_reference}/{item.file_path}";
            ...
            catch (Exception ex)
            {
                ...
                string dir = $"{_configuration["AppSettings:DOC_PATH"]}/Documents/Travel/{group_reference}";
                if (Directory.Exists(dir))
                {
        ...
        [HttpGet("{referrenceNo?}/{referrenceKey?}/{merchant_id?}/{hash?}")]
            ...
            try
            {
                ...
                using (var api = new CustodianAPI.PolicyServicesSoapClient())
                {
                    ...
                    dapper_core.TransactionState.Connection.State == System.Data.ConnectionState.Open)
                            ...
                            bool result = false;
                            // no need converting as i set it to false alreay
                            Boolean.TryParse(_configuration["AppSettings:IsDemoMode"], out result);
        ...
        public async Task<notification_response> GetLocalTravelQuote(LocalTravelView localTravel)
        {
            try
            {
                ...
                string jsonFilePath = Path.Combine(_hostingEnvironment.ContentRootPath, "Cert", "NigeriaStates.json");

                // Read the contents of the file
                string getFile = System.IO.File.ReadAllText(jsonFilePath);
